...
class MultiAgent:
    def __init__(
        ...
        self._track_rewards_dict = {}
        for agent in self.possible_agents:
            self._track_rewards_dict[agent] = collections.deque(maxlen=100)
        
        self._track_timesteps = collections.deque(maxlen=100)
        self._cumulative_rewards = None
        self._cumulative_timesteps = None
        
        self._cumulative_rewards_dict = None
    ...
    def write_tracking_data(self, timestep: int, timesteps: int) -> None:
        ...
        # debug
        for agent in self.possible_agents:
            self._track_rewards_dict[agent].clear()
    ...
    def record_transition(
        ...
        if self._cumulative_rewards_dict is None:
            self._cumulative_rewards_dict = {}
            for agent in self.possible_agents:
                self._cumulative_rewards_dict[agent] = torch.zeros_like(_rewards, dtype=torch.float32)
        ...
        # debug 
        for agent in self.possible_agents:
            self._cumulative_rewards_dict[agent].add_(rewards[agent])
        ...
        if finished_episodes.numel():
            ...
            for agent in self.possible_agents:
                self._track_rewards_dict[agent].extend(self._cumulative_rewards_dict[agent][finished_episodes][:, 0].reshape(-1).tolist())
            ...
            for agent in self.possible_agents:
                self._cumulative_rewards_dict[agent][finished_episodes] = 0
        ...
        if self.write_interval > 0:
            ...
            # record data of every agents
            track_rewards_dict = {}
            for agent in self.possible_agents:
                if len(self._track_rewards_dict[agent]) > 0:
                    track_rewards_dict[agent] = np.array(self._track_rewards_dict[agent])
                    self.tracking_data[f"Reward / {agent} / Total reward (max)"].append(np.max(track_rewards_dict[agent]))
                    self.tracking_data[f"Reward / {agent} / Total reward (min)"].append(np.min(track_rewards_dict[agent]))
                    self.tracking_data[f"Reward / {agent} / Total reward (mean)"].append(np.mean(track_rewards_dict[agent]))
